...
export interface FunnelDataItemOption
  extends FunnelStateOption,
    StatesOptionMixin<FunnelStateOption>,
...
export interface FunnelSeriesOption
  extends SeriesOption<FunnelStateOption>,
    FunnelStateOption,
    BoxLayoutOptionMixin,
    SeriesEncodeOptionMixin {
  type?: "funnel";
...
class FunnelSeriesModel extends SeriesModel<FunnelSeriesOption> {
  ...
  init(option: FunnelSeriesOption) {
    ...
    this.legendVisualProvider = new LegendVisualProvider(
      zrUtil.bind(this.getData, this),
      zrUtil.bind(this.getRawData, this)
    );
  ...
  getInitialData(
    this: FunnelSeriesModel,
    option: FunnelSeriesOption,
    ecModel: GlobalModel
  ): List {
  ...
  getDataParams(dataIndex: number) {
    ...
    params.percent = !sum
      ? 0
      : +(((data.get(valueDim, dataIndex) as number) / sum) * 100).toFixed(2);
